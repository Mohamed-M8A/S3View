...
class ReportingWriters:
    ...
    @staticmethod
    def write_all(standardized_entries, summary, is_simulation=False):
        ...
        if settings.get("REPORT_TSV", True):
            ReportingWriters._safe_write(
                ...
                standardized_entries, dirs["tsv"], f"{mode_prefix}_{timestamp}.tsv"
            )
        ...
        if settings.get("REPORT_SQLITE", False):
            ReportingWriters._safe_write(
                ...
                standardized_entries, dirs["sqlite"], "history.sqlite"
            )

        if settings.get("REPORT_DUCKDB", False):
            ReportingWriters._safe_write(
                ReportingWriters._write_duckdb, "duckdb",
                standardized_entries, dirs["duckdb"], "analytics.duckdb"
            )
