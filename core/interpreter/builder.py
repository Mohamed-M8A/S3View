...
class Builder:
    ...
    def build(script_content: str):
        ...
        while index < token_count:
            ...
            if index < token_count and tokens[index].type == TokenType.BLOCK_START:
                ...
                Builder._compile_to_pipeline(keyword, shared_metadata_raw, block_orders, loader, pipeline)
            else:
                inline_order, index = Builder._collect_inline_order(tokens, index, keyword)
                Builder._compile_to_pipeline(keyword, shared_metadata_raw, [inline_order], loader, pipeline)
    ...
    @staticmethod
    def _compile_to_pipeline(keyword, shared_meta_raw, orders, loader, pipeline):
        ...
        try:
            ...
            raise BuildError(f"METADATA_ERROR in '{keyword}': {exc}")
        ...
        for order_text, order_meta_raw in orders:
            if not order_text:
                raise BuildError(f"BUILD_ERROR: Command '{keyword}' contains an empty order.")
