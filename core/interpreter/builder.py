...
from core.models import CommandModel
...
class Builder:
    ...
    def _collect_inline_order(tokens, index, keyword):
        ...
        while index < token_count and tokens[index].type != TokenType.SEMICOLON:
            if tokens[index].type == TokenType.METADATA:
                ...
                index += 1
            elif tokens[index].type == TokenType.BANG:
                index += 1
                if index >= token_count or tokens[index].type != TokenType.STRING:
                    raise BuildError(f"SYNTAX_ERROR: Exclusion '!' in '{keyword}' must be followed by a quoted string.")
                while index < token_count and tokens[index].type == TokenType.STRING:
                    inline_exclusions.append(Builder._unquote(tokens[index].value))
                    index += 1
        ...
        index += 1
        return (" ".join(order_text_parts).strip(), specific_metadata, inline_exclusions), index

    @staticmethod
    def _unquote(string_token_value):
        text = string_token_value
    ...
    def _collect_block_scope(tokens, index, keyword):
        ...
        while index < token_count and tokens[index].type != TokenType.BLOCK_END:
            if tokens[index].type == TokenType.SEMICOLON:
                orders.append((" ".join(order_text_parts).strip(), specific_metadata, inline_exclusions))
                order_text_parts = []
                specific_metadata = ""
                inline_exclusions = []
                index += 1
        ...
        if order_text_parts or specific_metadata or inline_exclusions:
            orders.append((" ".join(order_text_parts).strip(), specific_metadata, inline_exclusions))
    ...
    def _compile_to_pipeline(keyword, shared_meta_raw, orders, loader, pipeline, keyword_line=None):
        ...
        for order_text, order_meta_raw, inline_exclusions in orders:
            if not order_text and method_name != "parse_nullary":
                raise BuildError(f"BUILD_ERROR: Command '{keyword}'{line_suffix} contains an empty order.")

            try:
                if order_meta_raw:
                    order_pkg = Metadata.extract(order_meta_raw)
                else:
                    order_pkg = MetadataPackage(
                        exclusions=shared_pkg.exclusions.copy(),
                        logic=shared_pkg.logic,
                        logic_inversion=shared_pkg.logic_inversion,
                        limit=shared_pkg.limit,
                        depth=shared_pkg.depth,
                        tier=shared_pkg.tier,
                        expires=shared_pkg.expires,
                        level=shared_pkg.level,
                        chunk_size=shared_pkg.chunk_size,
                        is_flat=shared_pkg.is_flat,
                        workers=shared_pkg.workers,
                        task_timeout=shared_pkg.task_timeout,
                        extra_metadata=shared_pkg.extra_metadata.copy(),
                    )
            except MetadataError as exc:
                raise BuildError(f"METADATA_ERROR in '{keyword}'{line_suffix}: {exc}")

            if inline_exclusions:
                order_pkg.exclusions = list(order_pkg.exclusions) + inline_exclusions
            ...
            if (plugin_info["plugin"].manifest.get("behavior", {}).get("dst_is_container")
                    and command_object.src is not None
                    and command_object.dst is not None
                    and command_object.src.is_directory
                    and not command_object.dst.is_directory):
                raise BuildError(
                    f"SYNTAX_ERROR: '{keyword}'{line_suffix}: the source '{command_object.src.payload}' is a "
                    f"folder (ends with '/'), but the destination '{command_object.dst.payload}' does not end "
                    f"with '/'. A folder source cannot be placed into a bare filename. Add a trailing '/' to "
                    f"the destination if you mean a folder, e.g. \"{command_object.dst.payload}/\"."
                )
