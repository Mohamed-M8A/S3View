...
class ActionParsers:
    ...
    def parse_bucket_name(command_text, metadata):
        """
        For account-level S3 operations (bucket create/purge) whose argument
        is a bare bucket NAME, not a filesystem/object path. Bypasses
        Paths.analyze()'s local-vs-cloud path heuristics -- which would
        otherwise resolve a bare name like "my-bucket" to a local filesystem
        path and route the command to the wrong connection manager entirely.
        """
