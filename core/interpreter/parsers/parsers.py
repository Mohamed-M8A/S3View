"""
core/interpreter/parsers/parsers.py -- the per-syntax-shape parsers that
turn a command's raw text (whatever follows its keyword) into a
CommandModel: parse_nullary (no path at all, e.g. "LB;"), parse_unary (one
path), parse_binary (source + "to" + destination), parse_reflective, and
parse_flexible.

parse_bucket_name (added 2026-08-30 session): for account-level S3
operations (bucket create/purge) whose argument is a bare bucket NAME, not
a filesystem/object path. It bypasses Paths.analyze()'s local-vs-cloud path
heuristics on purpose -- those would otherwise resolve a bare name like
"my-bucket" to a local filesystem path (since it has no "s3://" prefix and
no trailing slash to signal otherwise) and route the command to the wrong
connection manager entirely. This was a real, confirmed bug: both
`CREATE "new-bucket";` and `PURGE_BUCKET "old-bucket";`, exactly as shown
in their own manifest hints, crashed with "'LocalManager' object has no
attribute 's3_client'" before this fix, because the bucket name was being
treated as a relative local path instead of an S3 bucket identifier.
"""

from core.paths import Paths
from core.models import CommandModel, PathModel
...
class ActionParsers:
    ...
    def _split_by_keyword(text, keyword):
        ...
        while index < length:
            ...
            if not is_inside_quotes and text[index:index + keyword_length].upper() == keyword.upper():
    ...
    @staticmethod
    def parse_bucket_name(command_text, metadata):
        raw_text = (command_text or "").strip()
        if not ActionParsers._is_wrapped_in_quotes(raw_text):
            return {"error": f"SYNTAX_ERROR: Bucket name must be wrapped in double quotes -> {command_text}"}

        bucket_name = raw_text[1:-1].strip()
        if not bucket_name or "/" in bucket_name:
            return {"error": f"SYNTAX_ERROR: Invalid bucket name -> '{bucket_name}'"}
