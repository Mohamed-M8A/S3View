...
class FilterMatchers:
    ...
    def compile_rule(rule_string):
        ...
        if re.search(r"^size\s*" + numeric_pattern, rule_string):
            match = re.search(r"^size\s*" + numeric_pattern + r"\s*(.+)", rule_string)
            op, val = match.groups()
            return {"type": "size", "operator": op, "value": UnitParsers._execute_strict_parse(val.strip(), UnitParsers.SIZE_MAP, "SIZE")}
            
        if re.search(r"^sum\s*" + numeric_pattern, rule_string):
            match = re.search(r"^sum\s*" + numeric_pattern + r"\s*(.+)", rule_string)
            op, val = match.groups()
            return {"type": "sum", "operator": op, "value": UnitParsers._execute_strict_parse(val.strip(), UnitParsers.SIZE_MAP, "SIZE")}
            
        if re.search(r"^count\s*" + numeric_pattern, rule_string):
            match = re.search(r"^count\s*" + numeric_pattern + r"\s*(.+)", rule_string)
            op, val = match.groups()
            return {"type": "count", "operator": op, "value": float(val.strip())}
            
        if re.search(r"^age\s*" + numeric_pattern, rule_string):
            match = re.search(r"^age\s*" + numeric_pattern + r"\s*(.+)", rule_string)
            op, val = match.groups()
        ...
        if rule_string.startswith("ext" + separator) or rule_string.startswith("type" + separator):
            actual_category = rule_string.split(separator, 1)[0].strip()
            values = [v.strip().lstrip(".").lower() for v in rule_string.split(separator, 1)[1].split(",") if v.strip()]
            return {"type": "ext", "value": values, "source_keyword": actual_category}

        if rule_string.startswith("mime" + separator):
            values = [v.strip() for v in rule_string.split(separator, 1)[1].split(",") if v.strip()]
            return {"type": "mime", "value": values}

        for category in ["tier", "sname", "ename"]:
    ...
    def match_compiled(rule, metadata, stats):
        ...
        if rtype == "sum":
            if not stats.get("is_final", False):
                raise ValueError("FILTER_ERROR: 'sum' rules require a finalized dataset and cannot be evaluated during a streaming scan.")
            return FilterMatchers._compare_values(stats['total_size'], rule["operator"], rule["value"])
            
        if rtype == "count":
            if not stats.get("is_final", False):
                raise ValueError("FILTER_ERROR: 'count' rules require a finalized dataset and cannot be evaluated during a streaming scan.")
        ...
        if rtype == "ext":
            ...
            filename = metadata['key'].rsplit("/", 1)[-1]
            if "." not in filename: return False
            actual_ext = filename.rsplit(".", 1)[-1].lower()
            return any(actual_ext == target for target in rule["value"])
...
class LogicProcessor:
        ...
        for group_string in or_groups:
            ...
            for raw_rule in and_rules:
                ...
                if compiled_rule is None:
                    raise ValueError(f"FILTER_ERROR: Unrecognized filter rule -> '{actual_rule_text}'")
    ...
    @staticmethod
    def evaluate(compiled_groups, resource_metadata, is_inverted=False, total_size=0, total_count=0, is_final=False):
        ...
        pipeline_stats = {"total_size": total_size, "total_count": total_count, "is_final": is_final}
        match_result = False
...
class FilterEngine:
    ...
    @classmethod
    def should_process_compiled(cls, compiled_logic, object_metadata, is_inverted=False, total_size=0, total_count=0, is_final=False):
        return LogicProcessor.evaluate(compiled_logic, object_metadata, is_inverted, total_size, total_count, is_final)

    @staticmethod
    def requires_mime_resolution(compiled_logic):
        if not compiled_logic:
            return False
        for group in compiled_logic:
            for rule in group:
                if rule.get("type") == "mime":
                    return True
        return False