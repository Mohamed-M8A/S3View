# ==================================================================================
# SYSTEM & USER RESOURCE PROTECTION ENGINE (SHIELD)
# ------------------------------------------------------------------------------
# ARCHITECTURE & LOGIC:
# 1. Defensive Layers: Implements a dual-layer firewall. Layer 1 is hardcoded 
#    OS-critical paths (Kernel, System32, etc.). Layer 2 is a user-defined 
#    blacklist managed via "Protected.vshield".
# 2. Stringent Validation: The loader enforces a strict quote-wrapped syntax 
#    ("") for the vshield file. Any entry lacking double quotes is ignored 
#    to prevent accidental or malformed protection rules.
# 3. Strict Boundary Matching: Prevents partial-string collisions (e.g., "new" 
#    will no longer block "newone"). Protection only triggers on exact matches 
#    or explicit parent-child directory relationships.
# 4. Case-Sensitivity Logic: Automatically toggles between case-insensitive 
#    (Windows) and case-sensitive (Linux/S3) matching based on the platform.
# 5. Recursive Blocking: Protection is inherited; blocking a directory 
#    automatically cascades to all nested files and sub-directories.
# ==================================================================================
...
class Shield:
    ...
    def is_allowed(path_object, exclusions=None, protected_list=None):
        ...
        for danger_name in Shield.GENERIC_DANGEROUS_FOLDER_NAMES:
            if normalize_for_comparison(danger_name) in subject_segments: 
                return False
