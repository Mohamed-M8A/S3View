...
class Shield:
    ...
    def load_protected_list(force_reload=False):
        ...
        protected_entries = []
        vshield_file_path = Paths.resource_path("WORKSPACE/Protected.vshield")
    ...
    @staticmethod
    def is_allowed(path_object, exclusions=None, protected_list=None, protocol=None):
        ...
        subject_payload = normalize(current_payload)
        subject_filename = normalize(base_filename)
        subject_segments = [normalize(s) for s in path_segments]

        for exclusion in (exclusions or []):
            norm_excl = normalize(Paths.clean(exclusion).strip("/"))
        ...
        for protected_path in (protected_list or []):
            norm_protected = normalize(protected_path)
            if subject_payload == norm_protected or subject_payload.startswith(norm_protected + "/"):
                return False
            if norm_protected.startswith(subject_payload + "/"):
                return False
