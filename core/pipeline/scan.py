...
class UniversalScanner:
    ...
    def scan(connection_manager, path_object: PathModel, command, statistics):
        ...
        if not Shield.is_allowed({"payload": root_identifier}, command.exclusions, protected_list):
            return []
        ...
        if path_object.protocol in ("LOCAL", "EXTERNAL"):
    ...
    def _scan_s3_storage(manager, path_object: PathModel, command, statistics, protected_list):
        ...
        is_recursive_mode = target_prefix.endswith("/") or target_prefix == ""
        ...
        try:
            ...
            for page in paginator.paginate(Bucket=target_bucket, Prefix=target_prefix):
                ...
                for obj in page['Contents']:
                    ...
                    if not Shield.is_allowed({"payload": full_id}, command.exclusions, protected_list):
                        continue
    ...
    def _scan_local_filesystem(path_object: PathModel, command, statistics, protected_list):
        inventory = []
        physical_root = Paths.get_full_physical_path(path_object)
        is_folder_intent = path_object.payload.endswith("/")

        if not is_folder_intent:
            if os.path.isfile(physical_root):
                cleaned_file_path = Paths.clean(physical_root)
                if Shield.is_allowed({"payload": cleaned_file_path}, command.exclusions, protected_list):
                    try:
        ...
        base_directory = Paths.clean(physical_root).rstrip("/")
        ...
        for root, directories, files in os.walk(base_directory, topdown=True):
            ...
            directories[:] = [
                ...
                if Shield.is_allowed({"payload": Paths.join(current_directory, d)}, command.exclusions, protected_list)
            ]
            ...
            for filename in files:
                ...
                if not Shield.is_allowed({"payload": full_file_path}, command.exclusions, protected_list):
                    continue
