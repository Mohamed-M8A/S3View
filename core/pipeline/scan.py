...
class UniversalScanner:
    ...
    def scan(connection_manager, path_object: PathModel, command, statistics):
        ...
        # The root/system-path safety guard applies to every action EXCEPT
        # LIST. LIST is strictly read-only and cannot modify or destroy
        # anything, so there is no safety reason to block it from reading a
        # drive root or a system directory -- every other action (COPY,
        # MOVE, SYNC, DELETE, ARCHIVE, MEDIA, DELTA, ...) still goes through
        # the guard below, deliberately, since those can write or delete.
    ...
    def _scan_local_filesystem(path_object: PathModel, command, statistics, protected_list):
        ...
        # Guard against stripping the ONLY separator that makes a drive-root
        # or filesystem-root path unambiguous (e.g. "/" -> "" on POSIX, or
        # "D:/" -> "D:" on Windows, where "D:" means "current dir on drive D",
        # NOT the drive root -- an entirely different, ambiguous location).
