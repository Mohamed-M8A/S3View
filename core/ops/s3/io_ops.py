...
class IoOps:
    ...
    def s2s(manager, source_bucket, source_key, destination_bucket, destination_key, extra_args=None):
        ...
        if not is_encrypted(response) and source_etag and not is_multipart_etag(source_etag) and destination_etag:
            verify_etag(source_etag, destination_etag, context_label)
        else:
            verify_size(source_head.get("ContentLength"), source_head.get("ContentLength"), context_label)
    ...
    def s2l(manager, source_bucket, source_key, local_destination):
        ...
        if not is_encrypted(source_head) and source_etag and not is_multipart_etag(source_etag):
