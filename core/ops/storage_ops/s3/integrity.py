...
def compute_expected_etag(file_path, base_chunksize, multipart_threshold, file_size=None):
    ...
    if size <= multipart_threshold:
        _, hex_digest = _md5_of_range(file_path, 0, size)
        return hex_digest
    ...
    while offset < size:
        ...
        binary_digest, _ = _md5_of_range(file_path, offset, part_length)
        part_digests.append(binary_digest)
        offset += part_length

    concatenated = b"".join(part_digests)
    combined_hex = hashlib.md5(concatenated).hexdigest()
