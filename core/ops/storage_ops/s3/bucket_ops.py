class BucketOps:
    ...
    def _clean_bucket_name(bucket_name):
        return str(bucket_name).replace("s3://", "").replace("s3:/", "").split("/")[0].strip()
    ...
    def s3_create_bucket(manager, bucket_name):
        ...
        endpoint = manager.s3_client.meta.endpoint_url.lower()

        if "cloudflarestorage" in endpoint or "r2" in endpoint or region == "us-east-1":
            ...
            for page in paginator.paginate(Bucket=clean_name):
                ...
                if objects_to_delete:
                    manager.s3_client.delete_objects(
                        Bucket=clean_name, Delete={"Objects": objects_to_delete}
                    )
