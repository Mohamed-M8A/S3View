...
class PathResolver:
    ...
    def calculate_destination_key(source_key, base_path, destination_object: Optional[PathModel], command: CommandModel):
        ...
        clean_source = Paths.clean(source_key).strip("/")
        clean_base = Paths.clean(base_path or "").strip("/")

        if command.is_flat:
            relative_result = os.path.basename(clean_source)
        else:
            if not clean_base:
    ...
    def resolve_base_path(path_object: PathModel):
        if path_object.is_cloud:
            prefix = (path_object.prefix or "").strip("/")
            return prefix if prefix else ""

        return Paths.clean(Paths.get_full_physical_path(path_object)).strip("/")
