...
def run_logic_pipeline(is_cli=True, override_dry_run=None):
    ...
    try:
        ...
        if settings.get("ENABLE_REPORTS", True):
            ...
            if is_cli:
                if had_failures:
                    print("[!] TASK COMPLETED WITH ERRORS. See report/logs for details.")
                else:
                    print("[+] TASK COMPLETED SUCCESSFULLY.")

        if had_failures:
            return {"results": results, "network_logs": network_logs, "error": "One or more pipeline steps failed. See report/logs for details."}
