...
def run_logic_pipeline(is_cli=True, override_dry_run=None):
    ...
    try:
        ...
        try:
            pipeline_output = pipeline.execute_pipeline(pipeline_commands, is_dry, is_cli)
            results = pipeline_output.get("results", {})
            network_logs = pipeline_output.get("network_logs", [])
            had_failures = pipeline_output.get("had_failures", False)
        except Exception as e:
        ...
        if settings.get("ENABLE_REPORTS", True):
            ...
            if is_cli:
                if had_failures:
                    print("[!] TASK COMPLETED WITH ERRORS. See reports/logs for details.")
                else:
                    print("[+] TASK COMPLETED SUCCESSFULLY.")

        if had_failures:
            return {"results": results, "network_logs": network_logs, "error": "One or more pipeline steps failed. See reports/logs for details."}
