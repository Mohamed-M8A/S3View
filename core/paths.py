...
class Paths:
    ...
    def analyze(raw_path):
        ...
        # Capture the user's literal trailing-"/" intent *before* it gets
        # destroyed below. os.path.abspath()/os.path.normpath() silently
        # strip trailing separators (e.g. abspath("/tmp/x/") == "/tmp/x"),
        # so if we checked endswith("/") only *after* calling abspath, a
        # folder-intent path would always come back as False -- with no
        # actual signal left over to correctly detect it as a directory.
        ...
        # NOTE: is_directory is determined *strictly* by the trailing "/" the
        # user typed -- nothing else. There is deliberately NO disk lookup
        # (os.path.isdir) here to "smartly" reclassify a path that lacks a
        # trailing slash as a folder just because one happens to exist on
        # disk. A path with no trailing slash always means "a file with this
        # exact name"; if no such file exists, that is a normal, expected
        # not-found result -- not a cue to guess the user meant a directory.
    ...
    def resolve_local_path(relative_or_filename):
        """
        Resolves a bare filename/relative path (e.g. a default output name with
        no destination specified) into an absolute physical path, consistent
        with how Paths.analyze() resolves LOCAL payloads.
        """
