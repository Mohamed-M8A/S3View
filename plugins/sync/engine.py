...
def worker(connection_manager, execution_context, command_model):
    ...
    try:
        if protocol == "C2C":
            BasicOps.c2c(connection_manager, command_model.src.bucket, source_path, command_model.dst.bucket, destination_path, extra_args=s3_extra_arguments)
        elif protocol == "C2L":
            local_dest = CoreUtils.resolve_local_path(destination_path) if command_model.dst.protocol == "LOCAL" else destination_path
            BasicOps.c2l(connection_manager, command_model.src.bucket, source_path, local_dest)
        elif protocol == "L2C":
            BasicOps.l2c(connection_manager, source_path, command_model.dst.bucket, destination_path, extra_args=s3_extra_arguments)
        elif protocol == "L2L":
            local_dest = CoreUtils.resolve_local_path(destination_path) if command_model.dst.protocol == "LOCAL" else destination_path
...
def execute_logic(connection_manager, command_model: CommandModel, plugin_instance, is_simulation=False):
    ...
    destination_inventory_map = SyncService.get_destination_map(connection_manager, destination_object, command_model.depth)
    ...
    for item in source_inventory:
        ...
        if relative_path not in destination_inventory_map or SyncService.should_sync(item, destination_inventory_map[relative_path]):
            if is_simulation:
                full_source_path = TaskOrchestrator.format_full_identifier(item.key, source_object)
                full_dest_path = TaskOrchestrator.format_full_identifier(relative_path, destination_object)
    ...
    if not is_simulation and sync_queue:
        batch_output = ExecutionEngine.run_smart_task(connection_manager, command_model, worker, plugin_instance, False, pre_built_inventory=sync_queue)
