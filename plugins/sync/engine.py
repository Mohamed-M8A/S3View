...
from core.ops.composite.sync import SyncOps
...
def worker(connection_manager, execution_context, command_model):
    ...
    try:
        if protocol == "S2S":
            ...
            BasicOps.s2l(connection_manager, command_model.src.bucket, source_path, destination_path)
        elif protocol == "L2S":
            BasicOps.l2s(connection_manager, source_path, command_model.dst.bucket, destination_path, extra_args=s3_extra_arguments)
        elif protocol == "L2L":
            BasicOps.l2l(source_path, destination_path, move_mode=False)
