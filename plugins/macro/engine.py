...
def worker(connection_manager, execution_context, command_model):
    ...
    behavior_config = execution_context.get("behavior", {})
    max_depth = behavior_config.get("max_recursion_depth", 15)
    ...
    for task in executable_pipeline:
        ...
        needed_protocol = sub_cmd.src.protocol if sub_cmd.src else "LOCAL"
        if needed_protocol == "S3" and "S3" not in active_managers:
            active_managers["S3"] = S3Manager()
        
        current_mgr = active_managers.get(needed_protocol, active_managers["LOCAL"])

        if action_name == "macro":
            sub_alias = sub_cmd.src.payload
            is_safe, _ = MacroStore.is_recursion_safe(sub_alias, updated_history, max_depth)
            if not is_safe:
                continue
            sub_cmd.extra_metadata["__macro_history__"] = updated_history

        PipelineEngine._dispatch_task(action_name, sub_cmd, current_mgr, is_simulation)
