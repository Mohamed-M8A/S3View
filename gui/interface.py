...
class S3ViewAPI:
    ...
    def list_reports(self):
        json_directory = Paths.resource_path("reports/json")
    ...
    def get_report(self, filename):
        file_path = Paths.resource_path(f"reports/json/{filename}")
        try:
...
def launch_gui(debug_mode=True):
    index_html_path = os.path.abspath(Paths.resource_path("gui/web_ui/index.html"))
    
    if not os.path.exists(index_html_path):
        print(f"[!] Critical: Web UI not found at {index_html_path}")
        return
