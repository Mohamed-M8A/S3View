...
class SettingsHandler:
    ...
    def get_all_config():
        try:
            ...
            general_keys = config.DEFAULT_SETTINGS["GENERAL"].keys()
            reports_keys = config.DEFAULT_SETTINGS["REPORTS"].keys()
            
            return {
                "settings": {str(k): v for k, v in configuration_data.items() if k in general_keys or k in reports_keys},
                "credentials": {str(k): v for k, v in configuration_data.items() if k in config.DEFAULT_CREDS}
            }
        except Exception:
    ...
    def get_plugins_data():
        ...
        plugins_dict, _ = plugin_loader.discover_plugins()
        metadata_list = []
        
        for action_name, plugin_instance in plugins_dict.items():
    ...
    def read_workspace_file(filename):
        absolute_file_path = Paths.resource_path(f"WORKSPACE/{filename}")
    ...
    def save_workspace(payload):
        try:
            ...
            if payload.get("commands") is not None:
                commands_path = Paths.resource_path("WORKSPACE/Commands.view")
            ...
            if payload.get("protected") is not None:
                protected_path = Paths.resource_path("WORKSPACE/Protected.vshield")
